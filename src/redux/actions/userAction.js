...
import axiosWithout from "../../axiosWithout";
...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosWithout.post(`/login`, { email, password }, config);
    if (data && data.user) {
...
export const register = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosWithout.post(`/register`, userData, { config });
    if (data && data.user) {
      await localStorage.setItem("Udetails", JSON.stringify(data.user));
    }
    if (data && data.token) {
...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosWithout.post(`/password/forgot`, email, config);
...
export const verifyPhone = (otp) => async (dispatch) => {
  try {
    ...
    console.log('after verify from db',data);
